...
ALL_BOARDS := coptercontrol rexcaeli pipxtreme simposix
...
# RexCaeli doesn't have BU or EF targets so we need to
# filter them out to prevent errors on the all_flight target.
BU_BOARDS  := $(filter-out rexcaeli, $(BU_BOARDS))
EF_BOARDS  := $(filter-out rexcaeli, $(EF_BOARDS))
