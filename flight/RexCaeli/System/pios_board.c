...
/**
 * Configuration for LSM330 chip
 */
#if defined(PIOS_INCLUDE_LSM330)
#include "pios_lsm330.h"

static const struct pios_lsm330_acc_cfg pios_lsm330_acc_cfg = {
	.range    = PIOS_LSM330_ACCEL_4G,
};

static const struct pios_exti_cfg pios_exti_lsm330_gyro_cfg __exti_config = {
	.vector = PIOS_LSM330_Gyro_IRQHandler,
	.line = EXTI_Line2,
	.pin = {
		.gpio = GPIOB,
		.init = {
...
static const struct pios_lsm330_gyro_cfg pios_lsm330_gyro_cfg = {
	.exti_cfg = &pios_exti_lsm330_gyro_cfg,
	.range    = PIOS_LSM330_SCALE_500_DEG,
};
...
#endif /* PIOS_INCLUDE_FLASH_SECTOR_SETTINGS */
...
void PIOS_Board_Init(void) {
	...
	if (PIOS_SPI_Init(&pios_spi_flash_accel_gyro_id, &pios_spi_flash_accel_gyro_cfg)) {
		PIOS_Assert(0);
	}
#endif /* PIOS_INCLUDE_SPI */

#if defined(PIOS_INCLUDE_FLASH)
	PIOS_Flash_Jedec_Init(pios_spi_flash_accel_gyro_id, 0, &flash_w25x_cfg);
#if defined(PIOS_INCLUDE_FLASH_SECTOR_SETTINGS)
...
#if defined(PIOS_INCLUDE_LSM330) && defined(PIOS_INCLUDE_SPI)
	PIOS_SPI_SetClockSpeed(pios_spi_flash_accel_gyro_id, PIOS_SPI_PRESCALER_256);
	// Set up the SPI interface to the serial flash 
	PIOS_LSM330_Acc_Init(pios_spi_flash_accel_gyro_id,1,&pios_lsm330_acc_cfg);
	init_test = PIOS_LSM330_Acc_Test();

	PIOS_LSM330_Gyro_Init(pios_spi_flash_accel_gyro_id,2,&pios_lsm330_gyro_cfg);
	init_test = PIOS_LSM330_Gyro_Test();
	PIOS_SPI_SetClockSpeed(pios_spi_flash_accel_gyro_id, PIOS_SPI_PRESCALER_8);
#endif /* PIOS_INCLUDE_LSM330 */
