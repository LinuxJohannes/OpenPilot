...
void PIOS_I2C_EV_IRQ_Handler(uint32_t i2c_id)
{
	...
	uint8_t SReg_1 = i2c_adapter->cfg->regs->SR1;
	...
	//we just sent the address - EV6 in ref manual
		...
		if (bytes == 1 && reading && subaddress_sent) { //we are receiving 1 byte - EV6_3
			...
			__DMB();
			a = i2c_adapter->cfg->regs->SR2;      //clear ADDR after ACK is turned off
			if (a);
			...
			a = i2c_adapter->cfg->regs->SR2;      //clear the ADDR here
			if (a);
			__DMB();
...
void PIOS_I2C_ER_IRQ_Handler(uint32_t i2c_id)
{
	...
	if (SR1Register & 0x0700) {
		...
		SR2Register = i2c_adapter->cfg->regs->SR2;
		if (SR2Register);
