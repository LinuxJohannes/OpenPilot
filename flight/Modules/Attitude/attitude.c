...
static void AttitudeTask(void *parameters)
{
	...
	accel_test = PIOS_LSM330_Acc_Test();
	gyro_test  = PIOS_LSM330_Gyro_Test();
...
static int32_t updateSensorsCC3D(AccelsData * accelsData, GyrosData * gyrosData)
{
	...
	struct pios_lsm330_acc_data  lsm330_acc_data;
	struct pios_lsm330_gyro_data lsm330_gyro_data;
	...
	xQueueHandle lsm330_queue = PIOS_LSM330_Gyro_GetQueue();
	while ( samples == 0 ?
			(xQueueReceive(lsm330_queue, (void *) &lsm330_gyro_data, SENSOR_PERIOD) != errQUEUE_EMPTY) :
			(xQueueReceive(lsm330_queue, (void *) &lsm330_gyro_data, 0) != errQUEUE_EMPTY)) {
		x +=  lsm330_gyro_data.gyro_x;
		y += -lsm330_gyro_data.gyro_y;
		z += -lsm330_gyro_data.gyro_z;
		samples++;
	}

	if (samples == 0) {
		// Unfortunately if the LSM330D gyro ever misses getting read, then it
		// will not trigger more interrupts. In this case we must force a read
		// to kickstart it.
		PIOS_LSM330_ReadGyros(&lsm330_gyro_data);
		return -1;	// Error, no data
	}

	// No accel data available
	if(PIOS_LSM330_Acc_FifoElements() == 0) {
		return -1;
	}

	gyros[0] = ((float) x / samples) * PIOS_LSM330_GetGyroScale();
	gyros[1] = ((float) y / samples) * PIOS_LSM330_GetGyroScale();
	gyros[2] = ((float) z / samples) * PIOS_LSM330_GetGyroScale();

	uint8_t i = 0;
	x = y = z = 0;
	do {
		++i;
		samples = PIOS_LSM330_ReadAccel(&lsm330_acc_data);
		x +=  lsm330_acc_data.acc_x;
		y += -lsm330_acc_data.acc_y;
		z += -lsm330_acc_data.acc_z;
	} while ( (i < 32) && (samples > 0) );

	accels[0] = ((float) x / i) * PIOS_LSM330_GetAccelScale();
	accels[1] = ((float) y / i) * PIOS_LSM330_GetAccelScale();
	accels[2] = ((float) z / i) * PIOS_LSM330_GetAccelScale();

	gyrosData->temperature  = (float) lsm330_gyro_data.temperature;
	accelsData->temperature = (float) i;
